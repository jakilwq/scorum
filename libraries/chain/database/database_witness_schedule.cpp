...
//        for (auto itr = widx.begin(); itr != widx.end() && active_witnesses.size() < SCORUM_MAX_VOTED_WITNESSES;
//        ++itr)
//        {
...
//        return a->proposed_chain_props.account_creation_fee.amount <
//        b->proposed_chain_props.account_creation_fee.amount;
//    });
...
//            if (hfp.next_hardfork != std::get<0>(hf_itr->first) || hfp.next_hardfork_time !=
...
// FC_REFLECT(scorum::chain::witness_schedule::printable_schedule,
// (current_virtual_time)(num_scheduled_witnesses)(items))
